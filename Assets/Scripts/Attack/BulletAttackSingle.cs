...
public class BulletAttackSingle : MonoBehaviour, IAttackable
{
    ...
    private void OnTriggerEnter2D(Collider2D other)
    {
        if (_targetLayer == (_targetLayer | (1 << other.gameObject.layer)))
        {
            ...
            GetComponent<Animator>()?.SetTrigger("Detonate");
            GetComponent<IMoveable>().CanMove = false;

            Attack(target);

            IStats _launcherIStats = _launcher?.GetComponent<IStats>();
            _launcherIStats?.AddMana(_launcherIStats.ManaToAdd);

            ManaBar manaBar = _launcher?.GetComponentInChildren<ManaBar>();
            if (manaBar != null)
            {
                manaBar.UpdateManaBar(_launcher.GetComponent<CharacterStats>().Mana, _launcher.GetComponent<CharacterStats>().MaxMana);
            }
        }
    }
